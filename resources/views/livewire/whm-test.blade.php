<div>
    <p>Server 10: {{ count($server10['data']['acct']) }} accounts</p>
    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
        <table class="min-w-full divide-y divide-gray-300">
            ...
            <tr>
                ...
                <th scope="col" class="whitespace-nowrap px-2 py-3.5 text-left text-sm font-semibold text-gray-900">Suspended</th>
            </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white">
            @foreach($server10['data']['acct'] as $account)
                <tr>
    ...
    <p>Server 10: {{ count($server09['data']['acct']) }} accounts</p>
    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg mt-36">
        <table class="min-w-full divide-y divide-gray-300">
            <thead class="bg-gray-50">
            <tr>
                <th scope="col" class="whitespace-nowrap py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">Name</th>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
            @foreach($server09['data']['acct'] as $account)
                <tr>
                    <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm text-tkd-blue-800 font-semibold sm:pl-6">{{ $account['domain'] }}</td>
                    <td class="whitespace-nowrap px-2 py-2 text-sm text-gray-900">{{ $account['ip'] }}</td>
                    <td class="whitespace-nowrap px-2 py-2 text-sm text-gray-900">{{ $account['diskused'] }}</td>
